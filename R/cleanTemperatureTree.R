...
#'   that are not plausible. We have chosen to remove temperatures under 0°C and
#'   above 40°C, but the function allows the user to choose manually the range
#'   of temperature he regards as acceptable. Values outside the range are replaced
#'   with NA.
#' @param x Temperature in °C, as obtained by \code{convertTemperature}. A
#'   numeric vector.
...
cleanTemperatureTree <- function(x, lower.TTree = -10, higher.TTree = 40){
  x[x < lower.TTree]<- NA
  x[x > higher.TTree] <- NA
  return(x)
}
